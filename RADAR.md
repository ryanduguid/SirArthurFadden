...
The intended source is a reviewed, deliberately versioned observation output from the corpus builder in this same repository, described in [README.md](README.md) and [BUILD.md](BUILD.md). That corpus’s distinction between a superseded compilation, a current version with no published compilation, a title no longer in force, and a failed lookup is preserved here. This project is not a replacement corpus builder and must not treat derived corpus material as authorised legal text.
